...
public:
    PrivateData(QwtNullPaintDevice* p) : q_ptr(p), mode(QwtNullPaintDevice::NormalMode)
    {
    }
...
public:
    ...
    virtual void drawPolygon(const QPoint*, int, PolygonDrawMode) override;

    virtual void drawPixmap(const QRectF&, const QPixmap&, const QRectF&) override;

    virtual void drawTextItem(const QPointF&, const QTextItem&) override;

    virtual void drawTiledPixmap(const QRectF&, const QPixmap&, const QPointF& s) override;
 ...
 * @brief Constructor
 */
QwtNullPaintDevice::QwtNullPaintDevice() : m_engine(nullptr)
{
