...
public:
    PrivateData(QwtDateScaleEngine* p, Qt::TimeSpec spec)
        : q_ptr(p), timeSpec(spec), utcOffset(0), week0Type(QwtDate::FirstThursday), maxWeeks(4)
    {
    }
...
QwtDateScaleEngine::QwtDateScaleEngine(Qt::TimeSpec timeSpec)
    : QwtLinearScaleEngine(10), m_data(qwt_make_unique< PrivateData >(this, timeSpec))
{
}
...
QwtDateScaleEngine::alignDate(const QDateTime& dateTime, double stepSize, QwtDate::IntervalType intervalType, bool up) const
{
    ...
    case QwtDate::Week: {
        const QDate date = QwtDate::dateOfWeek0(dt.date().year(), d->week0Type);
