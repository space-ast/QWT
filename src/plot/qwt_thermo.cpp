...
void QwtThermo::drawLiquid(QPainter* painter, const QRect& pipeRect) const
{
    ...
    if (d->colorMap != nullptr) {
        ...
        if (!values.isEmpty()) {
            ...
            qwtDrawLine(painter,
                        from,
                        d->colorMap->color(interval.minValue(), interval.maxValue(), values[ 0 ]),
                        pipeRect,
                        liquidRect,
                        d->orientation);
        }
        ...
        for (int i = 1; i < values.size(); i++) {
            ...
            for (int pos = from + 1; pos < to; pos++) {
                ...
                qwtDrawLine(painter,
                            pos,
                            d->colorMap->color(interval.minValue(), interval.maxValue(), v),
                            pipeRect,
                            liquidRect,
                            d->orientation);
            }
            ...
            qwtDrawLine(painter,
                        to,
                        d->colorMap->color(interval.minValue(), interval.maxValue(), values[ i ]),
                        pipeRect,
                        liquidRect,
                        d->orientation);
