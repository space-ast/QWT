...
public:
    PrivateData(QwtPlotDirectPainter* p) : q_ptr(p), hasClipping(false), seriesItem(nullptr), from(0), to(0)
    {
    }
 ...
 */
QwtPlotDirectPainter::QwtPlotDirectPainter(QObject* parent) : QObject(parent), QWT_PIMPL_CONSTRUCT
{
}
...
void QwtPlotDirectPainter::drawSeries(QwtPlotSeriesItem* seriesItem, int from, int to)
{
    ...
    if (immediatePaint) {
        ...
        if (d->hasClipping) {
            d->painter.setClipRegion(QRegion(canvasRect) & d->clipRegion);
        } else {
...
bool QwtPlotDirectPainter::eventFilter(QObject*, QEvent* event)
{
    ...
    if (event->type() == QEvent::Paint) {
        ...
        if (d->seriesItem) {
            ...
            if (!doCopyCache) {
                qwtRenderItem(&painter, canvas->contentsRect(), d->seriesItem, d->from, d->to);
            }
