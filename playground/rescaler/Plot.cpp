...
public:
    ...
    virtual void draw(QPainter* painter, const QwtScaleMap&, const QwtScaleMap&, const QRectF& canvasRect) const override
    {
        ...
        painter->drawText(textRect, Qt::AlignBottom | Qt::AlignRight, m_text);
    }
...
public:
    ...
    explicit RectItem(Type type) : m_type(type)
    {
    }
...
Plot::Plot(QWidget* parent, const QwtInterval& interval) : QwtPlot(parent)
{
    ...
    for (int i = 0; i < numEllipses; i++) {
        const double x = interval.minValue() + qwtRand() % qRound(interval.width());
        const double y = interval.minValue() + qwtRand() % qRound(interval.width());
        const double r = interval.minValue() + qwtRand() % qRound(interval.width() / 6);
