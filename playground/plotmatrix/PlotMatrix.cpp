...
PlotMatrix::PlotMatrix(int numRows, int numColumns, QWidget* parent) : QFrame(parent)
{
    ...
    for (int row = 0; row < numRows; row++) {
        for (int col = 0; col < numColumns; col++) {
            ...
            for (int axisPos = 0; axisPos < QwtAxis::AxisPositions; axisPos++) {
                connect(plot->axisWidget(axisPos), SIGNAL(scaleDivChanged()), SLOT(scaleDivChanged()), Qt::QueuedConnection);
            }
...
void PlotMatrix::setAxisScale(int axis, int rowOrColumn, double min, double max, double step)
{
    int row = 0;
    int col = 0;
