...
{
public:
    LinearColorMap(int formatType) : QwtLinearColorMap(Qt::darkCyan, Qt::red)
    {
...
{
public:
    HueColorMap(int formatType) : QwtHueColorMap(QwtColorMap::Indexed)
    {
        setFormat((QwtColorMap::Format)formatType);
...
protected:
    virtual QImage
    renderImage(const QwtScaleMap& xMap, const QwtScaleMap& yMap, const QRectF& area, const QSize& imageSize) const QWT_OVERRIDE
    {
        QElapsedTimer t;
        t.start();

        QImage image = QwtPlotSpectrogram::renderImage(xMap, yMap, area, imageSize);
...
Plot::Plot(QWidget* parent) : QwtPlot(parent), m_alpha(255)
{
    ...
    const int extent = QwtPainter::horizontalAdvance(axisWidget(QwtAxis::YLeft)->fontMetrics(), "100.00");
...
void Plot::showSpectrogram(bool on)
{
    ...
    m_spectrogram->setDefaultContourPen(on ? QPen(Qt::black, 0) : QPen(Qt::NoPen));
...
void Plot::drawItems(QPainter* painter, const QRectF& canvasRect, const QwtScaleMap maps[ QwtAxis::AxisPositions ]) const
{
