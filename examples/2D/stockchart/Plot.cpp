...
{
public:
    Zoomer(QWidget* canvas) : QwtPlotAxisZoomer(canvas)
    {
...
{
public:
    DateScaleDraw(Qt::TimeSpec timeSpec) : QwtDateScaleDraw(timeSpec)
    {
...
public:
    ...
    void setInterval(const QDate& date1, const QDate& date2)
    {
        ...
        QwtPlotZoneItem::setInterval(QwtDate::toDouble(dt1), QwtDate::toDouble(dt2));
    }
};
}
...
void Plot::populate()
{
    ...
    const Qt::GlobalColor colors[] = { Qt::red, Qt::blue, Qt::darkCyan, Qt::darkMagenta, Qt::darkYellow };
...
{
    QwtPlotTradingCurve::SymbolStyle symbolStyle = static_cast< QwtPlotTradingCurve::SymbolStyle >(style);
