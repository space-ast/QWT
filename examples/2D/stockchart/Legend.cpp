...
LegendTreeView::LegendTreeView(Legend* legend) : QTreeView(legend)
{
...
QList< QStandardItem* > LegendTreeView::itemList(const QwtPlotItem* plotItem)
{
...
QSize LegendTreeView::sizeHint() const
{
    ...
    const QStandardItemModel* mdl = qobject_cast< const QStandardItemModel* >(model());
    ...
    for (int row = 0; row < mdl->rowCount(); row++) {
        ...
        int wRow = 0;
        for (int i = 0; i < rootItem->rowCount(); i++) {
            const QSize hint = delegate->sizeHint(styleOption, rootItem->child(i)->index());

            wRow = qMax(wRow, hint.width());
            h += hint.height();
        }

        const QSize rootHint = delegate->sizeHint(styleOption, rootItem->index());
...
Legend::Legend(QWidget* parent) : QwtAbstractLegend(parent)
{
    ...
    connect(m_treeView, SIGNAL(clicked(const QModelIndex&)), this, SLOT(handleClick(const QModelIndex&)));
}
...
void Legend::renderLegend(QPainter* painter, const QRectF& rect, bool fillBackground) const
{
    if (fillBackground) {
    ...
    const QStandardItemModel* mdl = qobject_cast< const QStandardItemModel* >(m_treeView->model());
...
void Legend::updateLegend(const QVariant& itemInfo, const QList< QwtLegendData >& legendData)
{
...
{
    const QStandardItemModel* model = qobject_cast< QStandardItemModel* >(m_treeView->model());
