...
public:
    ...
    SplineFitter(Mode mode) : QwtCurveFitter(QwtCurveFitter::Path), m_spline(NULL)
    {
        switch (mode) {
        case PleasingSpline: {
    ...
    void setClosing(bool on)
    {
        ...
        m_spline->setBoundaryType(on ? QwtSpline::ClosedPolygon : QwtSpline::ConditionalBoundaries);
    }
    ...
    virtual QPolygonF fitCurve(const QPolygonF& points) const override
    {
        return m_spline->polygon(points, 0.5);
    }

    virtual QPainterPath fitCurvePath(const QPolygonF& points) const override
    {
...
{
public:
    Curve(const QString& title, const QColor& color) : QwtPlotCurve(title)
    {
...
{
public:
    LineMarker(const QString& title, const QColor& color) : QwtPlotMarker(title)
    {
...
Plot::Plot(bool parametric, QWidget* parent) : QwtPlot(parent), m_boundaryCondition(QwtSplineC1::Clamped2)
{
    ...
    if (parametric) {
        ...
        points << QPointF(10, 30) << QPointF(20, 90) << QPointF(25, 60) << QPointF(35, 38) << QPointF(42, 40)
               << QPointF(55, 60) << QPointF(60, 50) << QPointF(65, 80) << QPointF(73, 30) << QPointF(82, 30)
               << QPointF(87, 40) << QPointF(95, 70);
    }
    ...
    connect(scalePicker, SIGNAL(clicked(int, double)), this, SLOT(updateMarker(int, double)));
    ...
    connect(m_wheel, SIGNAL(valueChanged(double)), SLOT(scrollLeftAxis(double)));

    // we need the resize events, to lay out the wheel
    canvas()->installEventFilter(this);

    m_wheel->setWhatsThis("With the wheel you can move the visible area.");
    axisWidget(QwtAxis::XBottom)->setWhatsThis("Selecting a value at the scale will insert a new curve.");
}
...
bool Plot::eventFilter(QObject* object, QEvent* e)
{
    if (e->type() == QEvent::Resize) {
        if (object == canvas()) {
            ...
            m_wheel->move(cr.right() - margin - m_wheel->width(), cr.center().y() - (m_wheel->height()) / 2);
        }
    }
