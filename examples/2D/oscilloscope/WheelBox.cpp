...
{
public:
    Wheel(QWidget* parent) : QwtWheel(parent), m_ignoreWheelEvent(false)
    {
    ...
    virtual bool eventFilter(QObject* object, QEvent* event) override
    {
        if (event->type() == QEvent::Wheel && !m_ignoreWheelEvent) {
            ...
            QWheelEvent wheelEvent(pos,
                                   ...
                                   we->inverted());
#else
            QWheelEvent wheelEvent(pos, we->delta(), we->buttons(), we->modifiers(), we->orientation());
#endif
...
WheelBox::WheelBox(const QString& title, double min, double max, double stepSize, QWidget* parent) : QWidget(parent)
{
    ...
    connect(m_wheel, SIGNAL(valueChanged(double)), m_number, SLOT(display(double)));

    connect(m_wheel, SIGNAL(valueChanged(double)), this, SIGNAL(valueChanged(double)));
}
