...
void Plot::setSettings(const Settings& s)
{
    ...
    if (s.curve.numPoints != buffer->size() || s.curve.functionType != m_settings.curve.functionType) {
    ...
    m_curve->setPaintAttribute(QwtPlotCurve::ClipPolygons, s.curve.paintAttributes & QwtPlotCurve::ClipPolygons);
    m_curve->setPaintAttribute(QwtPlotCurve::FilterPoints, s.curve.paintAttributes & QwtPlotCurve::FilterPoints);
    m_curve->setPaintAttribute(QwtPlotCurve::FilterPointsAggressive,
                               s.curve.paintAttributes & QwtPlotCurve::FilterPointsAggressive);

    m_curve->setRenderHint(QwtPlotItem::RenderAntialiased, s.curve.renderHint & QwtPlotItem::RenderAntialiased);
...
#endif
    {
        ...
        plotCanvas->setPaintAttribute(QwtPlotCanvas::BackingStore, s.canvas.useBackingStore);
        plotCanvas->setPaintAttribute(QwtPlotCanvas::ImmediatePaint, s.canvas.immediatePaint);
    }
