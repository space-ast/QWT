...
{
public:
    Knob(const QString& title, double min, double max, QWidget* parent = NULL) : QWidget(parent)
    {
...
protected:
    virtual void resizeEvent(QResizeEvent* event) override
    {
        ...
        m_knob->setGeometry(0, m_label->pos().y() - h + off, sz.width(), h);
    }
...
{
public:
    Thermo(const QString& title, QWidget* parent = NULL) : QWidget(parent)
    {
...
AmplifierBox::AmplifierBox(QWidget* p) : QFrame(p)
{
...
void AmplifierBox::timerEvent(QTimerEvent*)
{
    ...
    const double sig_bass   = (1.0 + 0.1 * m_knobBass->value()) * std::sin(13.0 * phase);
    const double sig_mim_l  = std::sin(17.0 * phase);
    const double sig_mim_r  = std::cos(17.5 * phase);
    const double sig_trbl_l = 0.5 * (1.0 + 0.1 * m_knobTreble->value()) * std::sin(35.0 * phase);
    const double sig_trbl_r = 0.5 * (1.0 + 0.1 * m_knobTreble->value()) * std::sin(34.0 * phase);

    double sig_l = 0.05 * m_master * m_knobVolume->value() * qwtSqr(sig_bass + sig_mim_l + sig_trbl_l);
    double sig_r = 0.05 * m_master * m_knobVolume->value() * qwtSqr(sig_bass + sig_mim_r + sig_trbl_r);
