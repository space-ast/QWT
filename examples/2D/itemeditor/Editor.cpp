...
{
public:
    Overlay(QWidget* parent, Editor* editor) : QwtWidgetOverlay(parent), m_editor(editor)
    {
        switch (editor->mode()) {
        case Editor::NoMask: {
...
Editor::Editor(QwtPlot* plot) : QObject(plot), m_isEnabled(false), m_overlay(NULL), m_mode(Mask)
{
    setEnabled(true);
}
...
bool Editor::eventFilter(QObject* object, QEvent* event)
{
    ...
    if (plot && object == plot->canvas()) {
        ...
        case QEvent::MouseButtonPress: {
            ...
            if (m_overlay == NULL && mouseEvent->button() == Qt::LeftButton) {
...
QRegion Editor::maskHint(QwtPlotShapeItem* shapeItem) const
{
    ...
    QRect rect = QwtScaleMap::transform(xMap, yMap, shapeItem->shape().boundingRect()).toRect();
...
void Editor::drawOverlay(QPainter* painter) const
{
    ...
    painter->setRenderHint(QPainter::Antialiasing, m_editedItem->testRenderHint(QwtPlotItem::RenderAntialiased));

    m_editedItem->draw(painter, xMap, yMap, plot->canvas()->contentsRect());
}
