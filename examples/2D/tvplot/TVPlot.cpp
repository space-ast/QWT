...
{
public:
    Histogram(const QString& title, const QColor& symbolColor) : QwtPlotHistogram(title)
    {
...
TVPlot::TVPlot(QWidget* parent) : QwtPlot(parent)
{
...
void TVPlot::populate()
{
    ...
    histogramJune->setValues(sizeof(juneValues) / sizeof(double), juneValues);
    histogramJune->attach(this);

    Histogram* histogramNovember = new Histogram("Winter", Qt::blue);
    histogramNovember->setValues(sizeof(novemberValues) / sizeof(double), novemberValues);
...
void TVPlot::setMode(int mode)
{
    ...
    for (int i = 0; i < items.size(); i++) {
        ...
        if (mode < 3) {
            ...
            symbol->setBrush(histogram->brush());
