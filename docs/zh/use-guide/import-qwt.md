...
- `core`模块，公共基础库，包含颜色映射（`QwtColorMap`）、调色板循环（`QwtColorCycle`）和色彩映射预设（`QwtColorMapPreset`）
- `plot`模块，主要为2D绘图库，是原qwt的功能集成
- `plot3d`模块，主要为3D绘图库，是qwtplot3d的功能集成

> **依赖说明**
>
>    `plot`和`plot3d`都依赖`core`模块。qwt依赖的Qt模块为`Core`、`Gui`、`Widgets`、`Svg`、`Concurrent`、`OpenGL`、`PrintSupport`，（如果是Qt6还会包含`OpenGLWidgets`），这些模块在引入`qwt`库时，会自动添加依赖
>
